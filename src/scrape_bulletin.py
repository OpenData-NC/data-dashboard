...
def parse_second(piece, id_and_type, officer):
...
#        id_and_type['record_id'] = hashlib.sha224( + matches[0][2] + matches[0][3] + matches[0][4] + matches[0][5]) \
    ...
    data = format_reported_date(data, 'occurred_date')
    return dict(data.items() + officer.items() + id_and_type.items() + other_data.items())
...
def on_between(data):
    ...
    if data['on_or_between'] == 'on':
        ...
        on_and_between['from_date'] = format_db_datetime(pieces[0].split(', ')[1])
        on_and_between['to_date'] = format_db_datetime(pieces[1].split(', ')[1])
    ...
    for date in dates:
        ...
        for row in soup.find_all('table', id="dgBulletin")[0].find_all('tr'):
            if count == 0:
                ...
                first_piece = dict(parse_first(pieces[0]).items() + {'agency': agency}.items())
                third_piece = {'reporting_officer': pieces[2].text}
                parse_second(pieces[1], first_piece, third_piece)
