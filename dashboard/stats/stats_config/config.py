...
#these dictionaries are used to customize sql queries and parts of stats.py
   
#tables specific to each county
counties = {
...
#these are county-specific tables, so we don't need to pass the county to the sql
no_county = ['dash_buncombe_real_estate','dash_nh_real_estate', 'dash_wake_real_estate']

#date fields we'll use by data type
date_fields = {
...
#select portion of sql by data type and query type
select_all = {
        'incidents': {
...
#any order or grouping needed by sql
...
#used to translate data type to more human-friendly text in API
return_types = {
        'incidents': 'Incidents',
        'arrests': 'Arrests',
